...
def doRegister(request):
    ...
    try:
        ...
        if request.POST['code'] != request.session['verifycode']:
            return redirect(reverse('mobile_register') + "?errinfo=2")
        ...
        if user.status == 1 or user.status == 6:
            ...
            if user.password_hash == md5.hexdigest():
                ...
                return redirect(reverse('mobile_register') + "?errinfo=5")
        else:
            return redirect(reverse('mobile_register') + "?errinfo=4")
    except Exception as err:
        print(err)
        return redirect(reverse('mobile_register') + "?errinfo=3")
...
def findway(request):
    ...
    site1 = request.GET.get("mobile_start_location", None)
    site2 = request.GET.get("mobile_arrival_location", None)
    tool = request.GET.get("mobile_tool", None)
    print(site1)
    print(site2)
    if tool == '地铁':
        ...
        while len(juli_sum_list) == 0:
            ...
            for i in juli_sum_dict:
                if i == juli_min:
                    ...
                    return render(request, 'mobile/subway.html', {'subway_dic': subway_dic})
    elif tool == '公交车':
        pos_location = {}  # 放进图里的所有节点 id 和经纬度数据
        loc = []  # 节点的经度和纬度
        Map = nx.Graph()
        ...
        while len(juli_sum_list) == 0:
            ...
            for i in juli_sum_dict:
                if i == juli_min:
                    ...
                    return render(request, 'mobile/bus.html', {'bus_dic': bus_dic})
    elif tool == '汽车':
        ...
        while len(juli_sum_list) == 0:
            ...
            for i in juli_sum_dict:
                if i == juli_min:
                    ...
                    return render(request, 'mobile/car.html', {'car_dic': car_dic})
    else: return render(request, 'mobile/info.html')
...
def research(request):
    ...
    count = str(7)
    xianzhi = str(1)
    return render(request, 'mobile/research.html', {'new_list':new_list,})
